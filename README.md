...
The answer — proven across **123 automated tests in 9 layers** against a live cluster — is **yes**.
...
```
123 passed · 0 failed · 0 skipped   (24s)
```
...
| L7 | **Snapshot Simulation** | 6 | `snapshot` | Point-in-time create/list/restore/delete via server-side S3 copy; restore re-verifies SHA-256, survives multi-snapshot coexistence and node restart |
...
| | **TOTAL** | **123** | | **All suites passing against live 4-node cluster (excludes `ai` marker when Ollama isn't running)** |
...
║  │    L7 Snapshot ─────────── point-in-time create/restore/delete    │   ║
...
- All 123 tests still pass — no test depends on Ollama unless it carries `@pytest.mark.ai`
...
│   │   └── test_security.py
│   ├── snapshots/                     L7 — 6 snapshot lifecycle tests
...
| AI graceful fallback | All 123 tests pass whether Ollama is running or not — AI is additive, not required |
...
python -m pytest -m snapshot                         # L7 — snapshot create/restore
